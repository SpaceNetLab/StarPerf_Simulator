...
def sunstorm_damaged_satellites():
    ...
    delay1 = DELAY.delay(source, target, dT, constellation.shells[4], ground_station_file, POP_file)
    print("\t\t\tThe delay time of original constellation from ", source.user_name, " to ", target.user_name, " for every timeslot are : ", delay1, " s")
    delay2 = DELAY.delay(source, target, dT, constellation_sunstorm_damaged.shells[4], ground_station_file, POP_file)
    ...
    coverage1 = COVERAGE.coverage(dT, constellation.shells[4], ground_station_file)
    print("\t\t\tThe coverage rates of every timeslot of the original constellation are ", coverage1)
    coverage2 = COVERAGE.coverage(dT, constellation_sunstorm_damaged.shells[4], ground_station_file)
    ...
    bandwidth1 = BANDWIDTH.bandwidth(source, target, dT, constellation.shells[4], ground_station_file)
    ...
    bandwidth2 = BANDWIDTH.bandwidth(source, target, dT, constellation_sunstorm_damaged.shells[4], ground_station_file)
