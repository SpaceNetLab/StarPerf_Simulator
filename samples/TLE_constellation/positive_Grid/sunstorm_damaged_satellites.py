...
In the constellation +Grid working mode, the satellite sunstorm damage model is used to destroy the constellation
...
def sunstorm_damaged_satellites():
    ...
    constellation_sunstorm_damaged = constellationDamageModelPluginManager. \
        execute_damage_model(constellation, constellation.shells[4], 0.05, dT, 230)
    ...
    delay2 = DELAY.delay(constellation_sunstorm_damaged.constellation_name, source, target, dT,
                         constellation_sunstorm_damaged.shells[4])  # 计算延迟
    ...
    minimum_path = routingPolicyPluginManager.execute_routing_policy(constellation_sunstorm_damaged.constellation_name,
                                                                     source, target,
                                                                     constellation_sunstorm_damaged.shells[4])
    ...
    coverage2 = COVERAGE.coverage(constellation_sunstorm_damaged.constellation_name, dT,
                                  constellation_sunstorm_damaged.shells[4])
    ...
    betweeness2 = BETWEENESS.betweeness(constellation_sunstorm_damaged.constellation_name,
                                        constellation_sunstorm_damaged.shells[4])
