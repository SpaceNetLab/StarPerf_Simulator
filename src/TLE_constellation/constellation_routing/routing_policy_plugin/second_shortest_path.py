...
Function : Use the sub-shortest path algorithm to calculate the path from source to destination target
...
def second_shortest_path(constellation_name , source , target , sh , t):
    ...
    # find the shortest path and delete the edges of the shortest path from the graph G
    minimum_path = nx.dijkstra_path(G, source=start_satellite, target=end_satellite)
    for x in range(1, len(minimum_path) - 2):
        G.remove_edge(minimum_path[x], minimum_path[x + 1])
    # find the second shortest path
    second_minimum_path = nx.dijkstra_path(G, source=start_satellite, target=end_satellite)

    return second_minimum_path