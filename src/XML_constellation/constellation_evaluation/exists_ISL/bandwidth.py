...
Function : This script is used to calculate the bandwidth between two ground stations

Implementation idea: First, pass in a shell in a constellation, as well as two ground station objects source and target,
...
# Function : Delete some nodes from the graph object G generated by networkx. The names of these nodes are stored in
#            nodes (nodes is a list type parameter)
...
def bandwidth(constellation_name , source , target ,sh, λ , isl_capacity , dT):
    ...
    path_num_per_timeslot = [0 for i in range((int)(sh.orbit_cycle / dT)+1)]
