...
def detect_orbit_number(raans):
    ...
    Using multiple advanced methods for maximum accuracy
    """
    if len(raans) < 10:
        return 1
    ...
    def ruptures_detection():
        ...
        for model in models:
            try:
                ...
                # Try different penalty strategies
                penalties = [
                ...
                for pen in penalties:
                    try:
                        ...
                        if len(changepoints) > 1:  # Exclude trivial case
    ...
    def histogram_detection():
        # Create adaptive number of bins
    ...
    def gap_analysis():
        ...
        # Use statistical tests to find significant gaps
        # Method: Find gaps that are outliers using z-score
        if len(gaps) > 0:
    ...
    def density_clustering():
        try:            
            ...
            eps = np.std(raans) * 0.1  # Adaptive epsilon
    ...
    try:
        ...
        all_results.extend([hist_result] * 2)  # Give it some weight
        ...
        all_results.extend([jenks_result] * 2)  # Give it more weight
        
    except Exception as e:
        print(f"Error in advanced detection: {e}")
    
    # Robust combination of results
    if all_results:
        ...
        filtered_results = all_results[(all_results >= lower_bound) & (all_results <= upper_bound)]
    ...
    # Final fallback: simple but reliable method
    sorted_raans = np.sort(raans)
    gaps = np.diff(sorted_raans)
